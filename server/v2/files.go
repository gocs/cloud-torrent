...
var (
	...
	ErrOverFileLimit  = errors.New("over file limit")
	ErrFileListing    = errors.New("file listing failed")
)
...
func list(path string, info os.FileInfo, node *fsNode, n *int) error {
	...
	if (*n) > fileNumberLimit {
		return ErrOverFileLimit //limit number of files walked
	}
	node.Name = info.Name()
	node.Size = info.Size()
